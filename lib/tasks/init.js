...
InitTask.prototype = {
    ...
    return new Promise(function (resolve, reject) {
      prompt.get(InitTask.QUESTION_CWD, function (err, res) {
        if (err) {
          ...
          var answer = res[InitTask.QUESTION_CWD][0];

          if (answer ===  'y') {
            resolve();
          } else {
            reject('Please run init inside desired directory');
          }
        }
      })
    });
  ...
  run: function () {
    ...
    return this.askCWD().then(function () {
      return self.copyStarterProjectToDir(process.cwd());
    });
  }
}
